...
### El browser tool automatizado no valida Service Workers

El browser tool automatizado (Codex/Playwright vía MCP) no puede validar Service Workers de forma confiable — falla con "An unknown error occurred when fetching the script" incluso para SW triviales, sin relación con el código del proyecto. Para cualquier verificación futura que involucre Service Workers, usar Chrome real a mano, no el browser automatizado.
...
7. ~~`dependencies` se guarda y se resuelve en la importación, pero no se visualiza ni bloquea nada.~~ Cerrada el 30 ago 2026 (1O, migración 014): en vez de descartarse por bajo uso (como estimated_cost), se implementó un badge visual — "Depende de tareas sin cerrar" en `TaskRow.tsx` cuando una tarea `in_progress` referencia otra que no está `done`. Sin bloqueo, solo aviso. `dependencies` se eliminó de `subtasks` (sin consumidor real ahí) y se mantiene únicamente en `tasks`. `taskStatusById` se calcula en `ProjectTasksClient` desde `workPlan.allTasks` (una dependencia puede apuntar a una tarea de otra fase) y se enhebra por `WorkSection` -> `TaskList` -> `TaskRow`.
...
32. ~~El registro del Service Worker falla en dev: `SW registration failed`, dos veces por carga, en toda ruta. El `sw.js` de la Fase 6B no se genera fuera de build. **Sin verificar en Vercel**: si también falla ahí, la PWA está rota en producción. Actualizado el 30 ago 2026 (1O): confirmado como bug activo, no solo deuda de fondo. `SwRegister.tsx` llama a `navigator.serviceWorker.register('/sw.js')` sin ninguna guarda, en cada navegación completa (no solo en el primer mount) — un `unregister()` manual no "queda": la siguiente carga de página lo vuelve a registrar desde cero. Reproducido en vivo: una condición de carrera entre ese re-registro y la carga de los chunks de webpack puede producir `TypeError: Cannot read properties of undefined (reading 'call')`, y en el peor caso deja el origen completo bloqueado para nueva navegación en esa pestaña (ni `/`, ni otras rutas, cargan — solo se recupera cerrando la pestaña o limpiando el Service Worker a mano).~~ Cerrada el 30 ago 2026 (1P), commit `50269b4`: `SwRegister.tsx` ahora hace return temprano si `process.env.NODE_ENV !== 'production'` (no registra nada en dev), y en producción consulta `navigator.serviceWorker.getRegistration()` antes de llamar a `.register()`, evitando registros duplicados en cada montaje/navegación — la condición de carrera con los chunks de webpack queda cerrada de raíz, no solo mitigada. Verificado con smoke test manual en Chrome: dev sin SW registrado, producción local (`npm run build && npm run start`) con un solo registration activo y log "SW already registered" en hard reload sin duplicados. Deploy Ready en Vercel confirmado.
...
41. reorderPhases (paso 3a, Etapa 3) no es atómica: un loop secuencial de updates individuales, sin transacción. Si falla a mitad de camino, las fases anteriores al fallo ya quedaron con su nuevo sort_order escrito y las posteriores no — inconsistencia acotada pero real. Misma familia que las deudas 26 y 30 (no-atomicidad de PostgREST en createPhase y moveTaskToPhase). Confirmado en 3b: reorderTasks heredó el mismo patrón secuencial no-atómico, mismo motivo (acotar la ventana de inconsistencia, no atomicidad real) — las dos funciones de reorder de la Etapa 3 comparten esta deuda por diseño, no por descuido.
...
2. ~~**`status` / `completed` en subtareas (8B).** Patch estricto con advertencia, sin derivación automática.~~ Cerrada el 30 ago 2026 (1O, migración 014): ya no aplica, la columna `completed` no existe más — la UI y los server actions leen/escriben `status` directamente, sin ningún campo paralelo que pueda desincronizarse.
